...
class SmartHrController extends Controller
{
    ...
    public function sendPayroll(Payroll $payroll, PayrollEmailService $mailService): RedirectResponse
{
    if ($payroll->isPaid()) {
        return back()->with('error', 'Bảng lương đã được thanh toán.');
    }
    ...
    return back()->with('success', 'Đã gửi bảng lương cho nhân viên.');
}

    public function confirmPayroll($token): View
{
    $payroll = Payroll::where('confirm_token', $token)->firstOrFail();

    if ($payroll->isPending()) {
        $payroll->markAsApproved();
    }

    return view('emails.payroll_confirm_success');
}

    public function payPayroll(Payroll $payroll, PayrollEmailService $mailService): RedirectResponse
{
    if (!$payroll->isApproved()) {
        return back()->with('error', 'Nhân viên chưa xác nhận bảng lương.');
    }
    ...
    return back()->with('success', 'Đã thanh toán lương.');
}
