...
<div class="container-fluid px-4">
    <div class="d-flex justify-content-between align-items-center mt-4 mb-3">
        ...
        <div class="d-flex gap-2">
            ...
            <a href="{{ route('hr-dashboard.export', ['format' => 'excel', 'period' => 'all']) }}" class="btn btn-outline-success btn-sm">
                <i class="bi bi-file-earmark-excel"></i> Xuất Excel
            </a>
            <a href="{{ route('hr-dashboard.export', ['format' => 'pdf', 'period' => 'all']) }}" class="btn btn-outline-danger btn-sm">
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-md-3">
            <div class="card border-success h-100">
                <div class="card-body text-center">
                    <div class="text-success fs-2 fw-bold">{{ $hrOverview['activeEmployees'] }}</div>
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-md-3">
            <div class="card h-100 border-danger">
                <div class="card-body text-center">
                    <div class="fs-4 fw-bold text-danger">{{ $contractStats['expiringSoon'] }}</div>
                    <div class="text-muted small">HĐ sắp hết hạn</div>
                </div>
            </div>
        </div>
    </div>
                ...
                <table class="table table-sm table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center"><span class="badge bg-primary">{{ $dept['count'] }}</span></td>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-4"><canvas id="contractTypeChart" height="200"></canvas></div>
                <div class="col-md-8">
                    <div class="table-responsive">
        ...
        <div class="card-body">
            <div class="row text-center">
                ...
                <div class="col-md-3">
                    ...
                    <div class="text-muted small">Đã tuyển</div>
                </div>
                <div class="col-md-3">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-hover mb-0">
                            <thead class="table-light">
                                <tr><th>Loại đơn</th><th class="text-center">Tổng</th><th class="text-center">Chờ duyệt</th><th class="text-center">Đã duyệt</th><th class="text-center">Từ chối</th></tr>
                            </thead>
                            <tbody>
                                <tr><td class="fw-medium">Nghỉ phép</td><td class="text-center">{{ $requestStats['totalLeave'] }}</td><td class="text-center"><span class="badge bg-warning text-dark">{{ $requestStats['pendingAll'] }}</span></td><td class="text-center"><span class="badge bg-success">{{ $requestStats['approvedAll'] }}</span></td><td class="text-center"><span class="badge bg-danger">{{ $requestStats['rejectedAll'] }}</span></td></tr>
                                <tr><td class="fw-medium">Tăng ca</td><td class="text-center">{{ $requestStats['totalOvertime'] }}</td><td></td><td></td><td></td></tr>
                                <tr><td class="fw-medium">Ứng lương</td><td class="text-center">{{ $requestStats['totalAdvance'] }}</td><td></td><td></td><td></td></tr>
                                <tr><td class="fw-medium">Hỗ trợ</td><td class="text-center">{{ $requestStats['totalSupport'] }}</td><td></td><td></td><td></td></tr>
                            </tbody>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <div class="row text-center">
                <div class="col-md-2"><div class="fs-3 fw-bold text-primary">{{ $accountStats['total'] }}</div><div class="text-muted small">Tổng</div></div>
                <div class="col-md-2"><div class="fs-3 fw-bold text-danger">{{ $accountStats['admin'] }}</div><div class="text-muted small">Admin</div></div>
                <div class="col-md-2"><div class="fs-3 fw-bold text-info">{{ $accountStats['hr'] }}</div><div class="text-muted small">HR</div></div>
                <div class="col-md-2"><div class="fs-3 fw-bold text-warning">{{ $accountStats['accountant'] }}</div><div class="text-muted small">Kế toán</div></div>
                <div class="col-md-2"><div class="fs-3 fw-bold text-secondary">{{ $accountStats['employee'] }}</div><div class="text-muted small">Nhân viên</div></div>
                <div class="col-md-2"><div class="fs-3 fw-bold text-success">{{ $accountStats['active'] }}</div><div class="text-muted small">Hoạt động</div></div>
            </div>
        </div>
    </div>

    {{-- ========== 9. BIẾU ĐỒ NHÂN VIÊN MỚI ========== --}}
    <div class="card mb-4">
    ...
    <div class="card mb-4">
        <div class="card-header bg-warning text-dark"><h6 class="mb-0"><i class="bi bi-exclamation-triangle"></i> Hợp đồng sắp hết hạn (30 ngày tới)</h6></div>
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-sm table-hover mb-0">
                    <thead class="table-light">
                        <tr><th>Nhân viên</th><th>Phòng ban</th><th>Loại HĐ</th><th>Ngày hết hạn</th><th class="text-center">Còn lại</th></tr>
                    </thead>
                    <tbody>
                        @foreach($expiringContracts as $c)
                        @php $daysLeft = now()->diffInDays($c->end_date, false); @endphp
                        <tr>
                            <td class="fw-medium">{{ $c->employee->name ?? 'N/A' }}</td>
                            <td>{{ $c->employee->department->name ?? '' }}</td>
                            <td><span class="badge bg-info">{{ $c->contract_type }}</span></td>
                            <td>{{ $c->end_date?->format('d/m/Y') }}</td>
                            <td class="text-center">
                                <span class="badge {{ $daysLeft <= 7 ? 'bg-danger' : ($daysLeft <= 14 ? 'bg-warning text-dark' : 'bg-secondary') }}">
                                    {{ $daysLeft }} ngày
                                </span>
                            </td>
                        </tr>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (pieCtx) new Chart(pieCtx, {
        ...
        data: {
            ...
            datasets: [{ data: @json($departmentStats['departments']->pluck('count')), backgroundColor: colors, borderWidth: 2, borderColor: '#fff' }]
        },
    ...
    if (barCtx) new Chart(barCtx, {
        ...
        data: {
            labels: @json($departmentStats['departments']->pluck('name')),
            datasets: [{ label: 'Số nhân viên', data: @json($departmentStats['departments']->pluck('count')), backgroundColor: '#0d6efd', borderRadius: 6 }]
        },
    ...
    if (trendCtx) new Chart(trendCtx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                data: @json($trendData),
                borderColor: '#0d6efd', backgroundColor: 'rgba(13,110,253,0.1)', fill: true, tension: 0.3, pointRadius: 4
            }]
        },
    ...
    if (deptPayCtx) new Chart(deptPayCtx, {
        type: 'bar',
        data: {
            labels: @json($deptPayLabels),
            datasets: [{ label: 'Tổng lương (triệu)', data: @json($deptPayData), backgroundColor: colors, borderRadius: 6 }]
        },
        options: { responsive: true, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, title: { display: true, text: 'Triệu VNĐ' } } } }
    });
    ...
    if (ctCtx) {
        ...
        new Chart(ctCtx, {
            type: 'doughnut',
            data: { labels: ctLabels, datasets: [{ data: @json($ctData), backgroundColor: colors.slice(0, 5), borderWidth: 2, borderColor: '#fff' }] },
    ...
    if (reqCtx) new Chart(reqCtx, {
        ...
        data: {
            ...
            datasets: [{ data: [{{ $requestStats['pendingAll'] }}, {{ $requestStats['approvedAll'] }}, {{ $requestStats['rejectedAll'] }}], backgroundColor: ['#ffc107', '#198754', '#dc3545'], borderWidth: 2, borderColor: '#fff' }]
        },
        options: { responsive: true, plugins: { legend: { position: 'bottom', labels: { boxWidth: 12 } } } }
    });
    ...
    if (neCtx) new Chart(neCtx, {
        type: 'bar',
        data: {
            labels: @json($newEmpLabels),
            datasets: [{ label: 'Nhân viên mới', data: @json($newEmpData), backgroundColor: '#198754', borderRadius: 6 }]
        },
