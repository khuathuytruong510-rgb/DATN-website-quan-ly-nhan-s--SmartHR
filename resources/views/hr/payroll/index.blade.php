    ...
    <div class="card shadow-sm border-0 mb-4">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center flex-wrap">
                <div>
                    <h3 class="fw-bold mb-1">Bảng lương nhân viên</h3>
                    <p class="text-muted mb-0">Quản lý và tính lương nhân viên theo tháng.</p>
                </div>
                ...
                <table class="table table-hover align-middle payroll-table mb-0">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center">Phút muộn</th>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                @if($payroll->working_days > $payroll->required_working_days)
                                    <span class="text-primary fw-semibold">{{ $payroll->working_days }}/{{ $payroll->required_working_days }}</span>
                            ...
                            {{-- Phút muộn --}}
                            <td class="text-center">
                                @if(($payroll->total_late_minutes ?? 0) > 0)
                                    <span class="badge bg-warning text-dark">{{ $payroll->total_late_minutes }} phút</span>
                                @else
                            ...
                            <td class="text-end text-danger fw-semibold">
                                @if(($payroll->late_deduction ?? 0) > 0)
                                    -{{ number_format($payroll->late_deduction) }}
                                @else
                                    0
                                @endif
                            </td>
...
.payroll-table thead th {
    ...
    letter-spacing:.5px; padding:15px 12px; border-bottom:2px solid #e9ecef; white-space:nowrap;
}
.payroll-table tbody td {
    padding:15px 12px; vertical-align:middle; white-space:nowrap;
    border-color:#f1f3f5; font-size:14px;
}
.payroll-table tbody tr { transition:all .18s ease; }
.payroll-table tbody tr:hover, .payroll-table tbody tr:hover td { background:#f8fbff; }
.badge { font-size:13px; font-weight:500; padding:6px 12px; border-radius:6px; }
.btn { border-radius:10px; font-weight:600; }
.form-control, .form-select { border-radius:10px; min-height:42px; }
.text-end { font-variant-numeric:tabular-nums; }
.text-success { font-weight:600; color:#198754!important; }
.text-primary { color:#0d6efd!important; }
.text-danger { font-weight:600; color:#dc3545!important; }
